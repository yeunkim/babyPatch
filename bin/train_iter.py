...
from run import run_two_stage_cnn_orig_truncatedloss, run_classify_weightedImg
...
# numslices = 10
    ...
    for ITER in np.arange(iterative):
        ...
        suffix = '2021_{0}slices'.format(numslices) + str(ITER+1)
        ...
        for ii in np.arange(iterations):
            ...
            solver = run_two_stage_cnn_orig_truncatedloss.Solver(fns_whole, epoch=epoch, lr=5e-4, f_dim=numchannels, batch_size=5000,
                                                                 in_features=1, labels=3, shuffle=True, pad=pad, channels=1, textfn = textfn, suffix=suffix,
                                                                 valobj=valfns_whole, spherecoord=False)
        ...
        for i in np.arange(len(fns)):
            ...
            # obj = '//{6}/{4}_{0}ch_initinterfeatimg_e{1}_i{3}_{5}_whole'.format(numchannels, epoch, i,smallestidx,
            #                                                                                           subjs['train'][i],suffix,objects_folder)
        ...
        if validation:
            for i in np.arange(len(valfns)):
                ...
                ## generate whole images
                # obj = '//{6}/{4}_{0}ch_valinitinterfeatimg_e{1}_i{3}_{5}_whole'.format(numchannels, epoch, i,
        ...
        if validation:
            ...
            label_OHE = solver.test(valrefineinterfeatimgs, valrefineoutputs, valniis, batchsize=1000, imgs=valobjs, uncertfn=valuncerts)
            del label_OHE
        ## time
        ...
        if third_model:
            ...
            refineoutputs = []
            for i in np.arange(len(objs)):
                refineinterfeatimg = '/{6}/{4}_{0}ch_refine2interfeatimg_added_e{1}_i{3}_{5}.nii.gz'.format(numchannels,
                                                                                                                     epoch,i,0,subjs['train'][i],suffix,outputs_folder)
            ...
            if validation:
                ...
                valrefineoutputs = []
                for i in np.arange(len(valobjs)):
                    valrefineinterfeatimg = '/{6}/{4}_{0}ch_valrefine2interfeatimg_added_e{1}_i{3}_{5}.nii.gz'.format(
                        numchannels, epoch, i, 0, subjs['val'][i], suffix,outputs_folder)
                ...
                label_OHE = solver.test(valrefineinterfeatimgs, valrefineoutputs, valniis, batchsize=1000, imgs=valobjs)
