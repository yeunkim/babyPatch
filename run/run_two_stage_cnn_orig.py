...
class Solver(object):
    def __init__(self, obj, valobj=None, epoch=30, batch_size=100000, lr=2e-4, f_dim =5, pad =3,
        ...
        if self.dropout:
            ...
            self.model = two_stage_cnn_uncertainty_orig.model_2input_mirrored(
                f_dim=self.f_dim,
                pad=self.pad,
                in_features=self.in_features,
                labels=self.labels,
            ...
            # self.model = two_stage_cnn.model(
            #     f_dim=self.f_dim,
            #     pad=self.pad,
            #     in_features=self.in_features,
            #     labels=self.labels,
            ...
            self.model = two_stage_cnn_orig.model_2input_mirrored(
                f_dim=self.f_dim,
                pad=self.pad,
                in_features=self.in_features,
                labels=self.labels,
        ...
        for e in range(self.epoch):
            ...
            for idx, (sample, indices, orig_indices, indx, indz, indy) in enumerate(self.dataloader):
                ...
                if self.diceloss:
                    ...
                    #TODO: check if optimizers are correct
                    self.optimizer.step()
                ...
                if idx != 0 and idx % 10 == 0:
                    ...
                    print('\n[{:02d}/{:d}] label_loss:{:.3f} label_loss_mod:{:.3f} total loss:{:.7f}'.format(
                        e+1,self.epoch, label_losses_cat, label_losses_mod_cat, total_losses_cat))
    ...
    def test(self, intimgname, intoutname, affine, dataloader= None, batchsize =1000, second_model=None,
             miscidx=None, correction_model=None, modifiedimgs=None):
        self.set_mode('eval')
        self.model.eval()
        X_list = []
        labeled_list = []
        ...
        for i in np.arange(datanum):
            ...
            for idx, (sample, indices, orig_indices, indx, indz, indy) in enumerate(ind_dataloader):
                ...
                if miscidx is not None:
                    ...
                    if self.iterative:
                        ...
                        label_OHE_mod, _, _ = self.model(neighmod, neighzmod, neighymod,neigh2, neigh2_z, neigh2_y)
                        neighmod, neighzmod, neighymod = self.modelWImg_1ch(neigh, neigh_z, neigh_y)
                        _, _, maxindx_mod = self.model_1ch(neighmod, neighzmod, neighymod)
                else:
                    neighbors = sample['neighbors']
                    neighbors_z = sample['neighbors_z']
                    neighbors_y = sample['neighbors_y']
                    ...
                    if self.iterative:
                        neighmod, neighzmod, neighymod = self.modelWImg(neigh, neigh_z, neigh_y)
                        label_OHE_mod, xhat_mod, maxindx_mod = self.model(neighmod, neighzmod, neighymod)
                ...
                if self.iterative:
                    self.neighmod = neighmod.detach() # neighmod.detach()
                    self.neighzmod = neighzmod.detach() # neighzmod.detach()
                    self.neighymod = neighymod.detach() # neighymod.detach()
                    # print(len(indx), len(indx[0]))
                    width = (2*self.pad)+1
                    ...
                    self.maxindxs_mod.append(maxindx_mod.detach().data)
                    modified_img /= denom_counter
                    maxindices_mod = torch.cat(self.maxindxs_mod, 0).cpu().numpy()
            ...
            if self.iterative:
                ...
                if self.uncertainty:
                    ...
                    suffix = 'init'
                nib.save(recon, filename=modifiedimgs[i])
