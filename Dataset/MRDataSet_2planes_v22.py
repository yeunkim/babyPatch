...
class ToTensor(object):
    ...
    def __call__(self, sample):
        label, neigh1, neigh2 = sample['label'], sample['neighbors1'], sample['neighbors2']
...
class MRDataSet(Dataset):
    ...
    def __init__(self, file, transform=None, pad=5, render=True, pkl = False, slices = True, channels=1, axes=(True,False,False),
        ...
        self.allslices = np.where(self.numslices == -1)[0]
        self.slices = slices
        self.planes = planes
        ...
        with h5py.File(self.file, 'r') as self.dataset:
            ...
            self.numslices[self.allslices] = np.abs(bounds[self.allslices[0]][1] - bounds[self.allslices][0][0])
            ...
            if self.slices:
                ...
                self.axis_locs = [random.sample(range(bounds[i][0], bounds[i][1]), self.numslices[i]) for i in
                                  [0, 1, 2]]
    ...
    def planes_to_patches(self, plane1, plane2, coords):
        i,j,k = coords
        finder = {
            0: self.dataset['data'][i - self.pad:i + self.pad + 1, j - self.pad:j + self.pad + 1, k],
            1: self.dataset['data'][i - self.pad:i + self.pad + 1, j, k - self.pad:k + self.pad + 1],
            2: self.dataset['data'][i, j - self.pad:j + self.pad + 1, k - self.pad:k + self.pad + 1]
        }
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.file, 'r') as self.dataset:
            i,j, k = self.idxs[:,idx]
            neigh1, neigh2 = self.planes_to_patches(self.plane1, self.plane2, (i,j,k))
            indices = ravel(self.idxs[:,idx],self.shape)
            datasetNum = self.dataset.attrs['datasetNum']
            label = self.dataset['targets'][i,j,k]
        sample = {'label': label}
        sample.update({'neighbors1': neigh1, 'neighbors2': neigh2})
