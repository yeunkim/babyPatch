...
###### models
initmodel = '/data/infant/vae_objs/002_1d_T2wT1w_bs_augaug_ants_norm_k512n_e5_lr5e4_3c_p3_yzxline5_nzy5_7_noz.obj'
refmodel = '/data/infant/vae_objs/002_1d_T2wT1w_bs_augaug_ants_norm_k512n_e5_lr5e4_3c_p3_yzxline5_nzy5_7_noz_3channels.obj'
...
###### generate refinment stage outptus

b = np.asarray(list(itertools.chain.from_iterable(solver.xhats)))
values = np.zeros([b.shape[0], 3])
for i in np.arange(len(b)):
    values[i] = b[i].data.cpu().numpy()
...
recon = nib.Nifti1Image(Y, affine=nii)
nib.save(recon, filename='/data/infant/processed/test/002_3channel_test_x_noz_b.nii.gz')

#### print out the refinement stage output
recon = nib.Nifti1Image(X, affine=nii)
nib.save(recon, filename='/data/infant/processed/test/002_3channel_test_predicted_noz_b.nii.gz')