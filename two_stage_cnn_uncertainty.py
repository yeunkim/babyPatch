...
class model_2input_mirrored(nn.Module):
    ...
    def forward(self, neigh, neigh_z, neigh_y, uncert, uncert_z, uncert_y, spherecoord=None):
        ...
        ## compute with uncertainty
        int_neighbors_uncertz = self.convinputz_uncert(uncert_z)
        neighbors_uncertz = self.int_convinputz_uncert(int_neighbors_uncertz)
        ...
        ## compute with uncertainty
        int_neighbors_uncerty = self.convinputy_uncert(uncert_y)
        neighbors_uncerty = self.int_convinputy_uncert(int_neighbors_uncerty)
        neigh_flat_uncerty = neighbors.view(neighbors_uncerty.size(0), -1)

        ycat = torch.cat((neigh_flaty, neigh_flat_uncerty),1)
        neigh_flat_y = self.uncerty(ycat)
        ...
        if self.spherecoords:
            f_spherecoord = torch.cat((f,spherecoord), 1)
