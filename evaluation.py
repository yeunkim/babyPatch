...
def maxhd(test, truth):
    c = 1
    test_tmp = torch.zeros_like(test)
    test_tmp[test == c] = 1

    truth_tmp = torch.zeros_like(truth)
    truth_tmp[truth == c] = 1
    ...
    ## comput hd
    try:
        result = medpy.metric.hd(test_tmp.cpu().detach().numpy(), truth_tmp.cpu().detach().numpy())
    except RuntimeError:
        result = 11
